...
import com.solusoftec.repositories.twitter.XComentarioRepository;
...
@Service
public class XComentarioService {

    @Autowired
    private XComentarioRepository comentarioRepository;
    private static final String BASE_VIDEO_URL = "http://localhost:8080/X/videos/watch/";
    private static final String BASE_IMG_URL = "http://localhost:8080/X/imagenes/watch/";
    public List<XComentario> findAll() {
        return comentarioRepository.findAll();
    }
    public Optional<XComentario> findById(Integer id) {
        return comentarioRepository.findById(id);
    }
    public XComentario save(XComentario comentario) {
    ...
    private static final Path BASE_IMG_PATH = Paths.get("D:", "Joeseep", "web_scrap", "Selenium_scrape", "twitter_scrape", "imagenes_descargadas");
    public Page<ComentarioXDTO> obtenerComentariosPorPublicacion(Integer publicacionId, int page, int size) {
        PageRequest pageRequest = PageRequest.of(page, size);
        Page<XComentario> comentarios = comentarioRepository.findByPublicacionId(publicacionId, pageRequest);
        // Convertir la lista de comentarios a DTOs
        return comentarios.map(comentario -> new ComentarioXDTO(
                comentario.getId(),
                comentario.getDescripcionComentario(),
                comentario.getUsuario(),
                comentario.getFecha(), // Asegúrate de que el formato de fecha sea correcto
                comentario.getUsuarioEnlace()
        ));
    }
